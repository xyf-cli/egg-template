...
class CartService extends Service {
  ...
  async _fillCartProduct(list) {
    ...
    return { list, total_price: totalPrice };
  }
  ...
  async createAction(params) {
    ...
    if (hasCart) {
      ...
      const d = [{ product_id: params.product_id, cart_num: params.cart_num, is_checked: !!params.is_checked }];
      cart.user_id = userId;
  ...
  async indexAction() {
    ...
    const cartDataStr = cart.getDataValue('cart_data');
    const cartData = JSON.parse(cartDataStr);
    const result = await this._fillCartProduct(cartData);
    return result;
  }
